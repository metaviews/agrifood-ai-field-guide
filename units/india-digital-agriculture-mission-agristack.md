...
claim-type: example (deployment) + framework (state-DPI architecture)
activity-status: deployed
critical-voice: food-sovereignty (state-stewarded data layer across farmer registry, plot registry, crop-sown registry — the corpus's first DPI-grade deployment)
capital-intensity: mixed (state-funded, designed for both smallholder interface and enterprise-grade data layer)
...
3. **AI-based monsoon onset forecasting pilot** (Kharif 2025)
...
4. **YES-TECH, CROPIC, PMFBY WhatsApp Chatbot**
...
1. **It is the corpus's first deployment-scale state-DPI unit**. The data-governance tag (`state-stewarded`) and data-rights-framework tag (`state-stewarded`) are now anchored in a real, named, scaled deployment — not a policy declaration. A talk on state-DPI vs vendor-owned vs cooperative data governance can use this unit as the canonical state-DPI case.
2. **The World Bank Connect4Impact DPI paper (Feb 2026) treats India's agrifood DPI as exportable as a model** — a structural observation worth surfacing. The unit pairs naturally with `indiaai-mission-agrifood.md` (the policy frame) and with `cropin-india.md` (the private-platform layer) to name the three layers of India's agrifood AI architecture.
3. **The Kisan e-Mitra deployment figure (9.3M queries, 11 languages, 8,000 queries/day) is the corpus's largest single-state-AI-deployment chatbot footprint**. Worth surfacing in talks about voice-first / multilingual AI in low-literacy contexts — the deployment pattern is materially distinct from corporate chat assistants.
...
- gaps: **G-070** (Independent verification of AgriStack / DAM's 7.63 crore farmer ID and 23.5 crore plot-survey figures against state audit or third-party census); **G-072** (Indian quick-commerce AI deployment at named scale — out of scope for this unit but cross-referenced)
- contested-claims: **C-052** (whether India's state-DPI agrifood AI model is exportable to other South Asian / SSA contexts — World Bank Connect4Impact says yes, Huda et al. 2026 frames other countries as TAM-driven individual-level adoption)
- related-units: `cropin-india.md` (private-platform anchor), `itc-maars-india.md` (conglomerate-phygital anchor), `indiaai-mission-agrifood.md` (policy frame co-anchor), `india-agrifood-ai-pattern.md` (meta-pattern unit), `indigenous-data-sovereignty.md` (critical-voice comparator), `joindata-netherlands.md` (cooperative-data-governance comparator — different model)

## Freshness

- Sources verified through 2026-07-19.
- PIB AI-and-Agriculture Explainer dated 2026-02-14.
- DAM Cabinet approval September 2, 2024.
- Bharat-VISTAAR announced Union Budget 2026-27 (Feb 2026).
- Biometric Update coverage Feb 2026.
- World Bank Connect4Impact DPI paper Feb 2026.
- Last-verified: 2026-07.
