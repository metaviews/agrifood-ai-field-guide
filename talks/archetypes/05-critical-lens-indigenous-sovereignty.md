...
- *"Is IPES-Food / Civil Eats aligned with IDSov?"* → IPES-Food's critique is parallel — corporate concentration, smallholder dispossession — but uses a different analytical framework (food sovereignty rather than data sovereignty). Civil Eats is journalism, not a framework. Worth distinguishing: parallel critique, different frameworks.
