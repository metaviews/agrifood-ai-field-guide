---
title: 'Quotes and Paraphrased Positions — Index'
---

# Quotes and Paraphrased Positions — Index

This is the filterable index of all entries in the `quotes/` directory. See `quotes/README.md` for the schema and maintenance discipline.

## Quick navigation

- [By speaker](#by-speaker) — alphabetical list of all speakers
- [By region](#by-region) — geographic distribution
- [By theme](#by-theme) — analytical tags
- [By freshness](#by-freshness) — current vs historical vs verify-on-next-cycle
- [By intended-use-in-talk](#by-intended-use-in-talk) — primary quote vs framing support

---

## By speaker

| Speaker | Role | Organization | Entry | Type | Freshness |
|---|---|---|---|---|---|
| Andrew Nelson | Farmer | (independent, Garfield WA) | [proprietary-data-reluctance](producers/nelson-andrew-proprietary-data-reluctance.md) | verbatim | current |
| Andre Zhu | Senior VP | Pinduoduo | [digital-inclusion](industry-executives/zhu-andre-pinduoduo-digital-inclusion.md) | verbatim | current |
| Brian Leake | Spokesperson | Bayer (Climate FieldView) | [farmers-own-data](industry-executives/leake-brian-bayer-farmers-own-data.md) | verbatim | current |
| Djessie Donkers | Advocacy group | ZLTO | [joindata-vendor-fragmentation](producers/donkers-djessie-joindata-vendor-fragmentation.md) | verbatim | current |
| Elizabeth Vaughan | Senior Manager | CAFF Small-Farm Tech Hub | [small-farm-vs-amazon](institutional-leaders/vaughan-elizabeth-small-farm-vs-amazon.md) | verbatim | current |
| GIDA | (consortium) | Global Indigenous Data Alliance | [care-principles-mission](institutional-mission-statements/care-principles-mission.md), [care-directs-us-home](institutional-mission-statements/gida-care-directs-us-home.md) | mission | current |
| Hervé Pillaud | Co-founder | La Ferme Digitale | [unicorns-not-in-agriculture](institutional-leaders/pillaud-herve-unicorns-not-in-agriculture.md), [culture-of-probability](institutional-leaders/pillaud-herve-culture-of-probability.md), [commons-and-mutual-model](institutional-leaders/pillaud-herve-commons-and-mutual-model.md) | verbatim (×3) | current |
| IEEE SA | Standards body | IEEE Standards Association | [ieee-2890-mission](institutional-mission-statements/ieee-2890-mission.md) | mission | current |
| IVADO | Implementation partner | IVADO | [ai-all-strategy-statement](institutional-mission-statements/ivado-ai-all-strategy-statement.md) | mission | current |
| Jay Willmot | Founder / CEO | Haven Greens | [local-demand](industry-executives/willmot-jay-haven-greens-local-demand.md) | verbatim | current |
| Jeff Torrie | Dairy farmer | (independent, Grey County ON) | [lely-succession](producers/torrie-jeff-lely-succession.md) | verbatim | historical (2018) |
| JoinData | Co-op | JoinData | [mission](institutional-mission-statements/joindata-mission.md) | mission | current |
| Mathé van den Bosch | Dairy farmer | (independent, NL) | [joindata-milk-data](producers/van-den-bosch-mathe-joindata-milk-data.md) | verbatim | current |
| Naïo COO Sarlandie / Matteo Vanotti | COO / Founder | Naïo Technologies / xFarm | [naming-concern / farmer-origin](industry-executives/naio-coo-sarlandie-naming-concern.md) | paraphrased | historical |
| Nicolas Fratantuono | Vineyard operator | (independent, BE) | [naio-vineyard-damage](producers/fratantuono-nicolas-naio-vineyard-damage.md) | verbatim | current |
| OADA | Open-source project | Open Ag Data Alliance | [mission-statements](institutional-mission-statements/oada-mission-statements.md) | mission | current |
| Peng Bin | Founder / CEO | XAG | [pesticide-exposure](industry-executives/peng-bin-xag-pesticide-exposure.md) | verbatim | historical (2018) |
| Farm Credit Canada | Crown corp | FCC | [ecosystem-not-technology-framing](institutional-mission-statements/fcc-ecosystem-not-technology-framing.md) | mission | current |
| Ag Data Transparent | Non-profit certification | Ag Data Transparent | [principles](institutional-mission-statements/ag-data-transparent-principles.md) | mission | current |
| Suresh Neethirajan | University Research Chair | Dalhousie | [ai-strategy-stops-at-city-limits](researchers-and-experts/neethirajan-suresh-ai-strategy-stops-at-city-limits.md), [green-ai-cybersecurity](researchers-and-experts/neethirajan-suresh-green-ai-cybersecurity.md) | paraphrased | current |
| Raffi Krikorian | CTO | Mozilla Foundation | [open-source-turning-point](researchers-and-experts/mozilla-krikorian-open-source-turning-point.md) | verbatim | current |
| Yuan Zhang | Head of Global Sales | DJI Agriculture | [essential-farm-equipment](industry-executives/zhang-yuan-dji-essential-farm-equipment.md) | paraphrased | current |

---

## By region

### NA-Canada
- Andrew Nelson (NA-US — also relevant cross-border)
- Elizabeth Vaughan (NA-US — also relevant)
- IVADO statement (June 2026)
- FCC ecosystem-not-technology framing (May 2026)
- Jay Willmot (Haven Greens, May 2026)
- Jeff Torrie (Grey County ON, 2018 — historical)
- Suresh Neethirajan (Dalhousie, 2026)

### NA-US
- Andrew Nelson (proprietary data reluctance)
- Brian Leake (Bayer)
- Elizabeth Vaughan (CAFF)
- Ag Data Transparent certification framework

### EU-France
- Hervé Pillaud (La Ferme Digitale) — three verbatim quotes
- Naïo Technologies COO Sarlandie

### EU-Netherlands
- Djessie Donkers (ZLTO)
- Mathé van den Bosch (JoinData member)
- JoinData mission statement

### EU-other (Belgium)
- Nicolas Fratantuono (Naïo JO robot vineyard)

### East-Asia
- Andre Zhu (Pinduoduo)
- Peng Bin (XAG)
- Yuan Zhang (DJI)

### Global (international / multi-region)
- GIDA CARE Principles
- GIDA "CARE Directs Us Home"
- IEEE 2890-2025
- OADA mission statements

---

## By theme

### Data governance / data rights
- Nelson (proprietary data reluctance)
- Donkers (vendor fragmentation)
- van den Bosch (JoinData milk data)
- Leake (Bayer farmers-own-data)
- Vaughan (small-farm-vs-Amazon)
- CARE Principles
- IEEE 2890-2025
- OADA mission
- JoinData mission
- Ag Data Transparent principles

### Cooperative / commons
- Pillaud (3 quotes: unicorns, culture of probability, commons-and-mutual-model)
- Donkers
- van den Bosch
- JoinData mission
- OADA mission

### AI deployment
- Willmot (Haven Greens)
- Zhang (DJI)
- Sarlandie / Vanotti (Naïo / xFarm farmer-origin framing)
- Neethirajan (Green AI / Cybersecurity)

### Worker conditions / succession
- Peng Bin (XAG pesticide exposure)
- Torrie (Lely succession)

### Vendor deployment specifics
- Fratantuono (Naïo vineyard damage)
- Sarlandie / Vanotti (farmer-origin)
- Zhang (DJI essential farm equipment)

### Food sovereignty
- Vaughan (community-food-system)
- Willmot (Canadian local demand)

### Indigenous data sovereignty
- CARE Principles (GIDA)
- IEEE 2890-2025
- GIDA "CARE Directs Us Home"

### Policy framework
- IVADO statement (Canada AI for All)
- FCC ecosystem-not-technology framing
- Neethirajan (AI strategy stops at city limits; Green AI; Cybersecurity as critical infrastructure)

### Funding models
- Pillaud (unicorns do not exist in agriculture)

---

## By freshness

### Current (within 18 months of July 2026)
- All 2026 entries
- Pre-2024 entries with current context (Bayer via Civil Eats March 2026)

### Historical (older, kept for context)
- Torrie (2018 — Grey County ON dairy farmer; substantive succession framing)
- Peng Bin (2018 — origin narrative; XAG context)
- Naïo COO Sarlandie / Vanotti (2022 WGCIT interview)

### Verify-on-next-cycle (none currently flagged)
- Empty — to be added in next refresh

---

## By intended-use-in-talk

### Primary quotes (suitable for direct quotation)
- Most verbatim quotes in this index

### Framing support (paraphrased positions, not for direct quotation)
- Neethirajan (academic-policy framing)
- Zhang (DJI)
- Naïo / xFarm farmer-origin
- Various institutional mission statements

### Aggregate observations

- **Producers (5 entries):** Andrew Nelson, Djessie Donkers, Mathé van den Bosch, Jeff Torrie, Nicolas Fratantuono. Five concrete producer voices — worth noting that the field guide has *more* producer voices than I expected at the start. Could expand further.
- **Industry executives (6 entries):** Willmot, Zhu, Peng Bin, Zhang, Leake, Naïo/xFarm. Six industry voices, predominantly vendor framing. The Leake / Nelson pair is the most useful for talk constructions — vendor framing + producer reality side-by-side.
- **Institutional leaders (4 entries):** Three Pillaud verbatim quotes + Vaughan (CAFF). Substantive analytical voices.
- **Researchers / experts (3 entries):** Neethirajan's two complementary framings + Mozilla CTO Raffi Krikorian. Single researcher (Neethirajan) plus Mozilla institutional voice; could expand further.
- **Institutional mission statements (8 entries):** CARE Principles, IEEE 2890-2025, OADA, JoinData, IVADO, FCC, ADT, GIDA communique. These are not personal quotes but institutional positions — useful for talks about frameworks.

## How this index evolves

- **Adding a new quote:** Write a new entry in the appropriate category subdirectory; update this index.
- **Refreshing a quote:** Update both the quote file and this index.
- **Removing a quote:** Mark as `freshness: historical` or remove entirely. Update both.

## Note on coverage gaps

The field guide currently has:

- **Producers well-represented** in NA-US (Nelson, Vaughan context), NA-Canada (Torrie), EU-Netherlands (Donkers, van den Bosch), EU-other (Fratantuono). Under-represented in East-Asia, LATAM, SSA, South-Asia, Oceania, Indigenous-led contexts. Worth noting as a coverage gap.

- **Industry executives** predominantly vendor framing. Counter-frames (cooperative / commons, Indigenous-led) are in institutional leaders and mission statements rather than industry-executives. That's a structural feature of who has voice in vendor marketing vs. cooperative / commons contexts.

- **Researchers / experts** currently anchored to Neethirajan and Mozilla CTO Krikorian. Could expand with researchers from France (INRAE), China (CGIAIR-side), LAC, IDSov context (GIDA-affiliated researchers).
